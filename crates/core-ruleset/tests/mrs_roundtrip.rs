...
fn domain_mrs_round_trip_matches_mihomo_semantics() {
    ...
    //
    // 命中：
    assert!(matcher.matches("baidu.com", None, None, None), "exact baidu.com");
    assert!(matcher.matches("BAIDU.COM", None, None, None), "case insensitive");
    assert!(matcher.matches("qq.com", None, None, None));
    assert!(matcher.matches("www.bilibili.com", None, None, None));
    // +.example.com → 任何子域命中（含本域）
    assert!(matcher.matches("example.com", None, None, None), "+.example.com base");
    assert!(matcher.matches("a.example.com", None, None, None), "+.example.com sub");
    assert!(matcher.matches("a.b.c.example.com", None, None, None), "+.example.com deep");
    // +.cn → 任何 .cn 域名
    ...
    assert!(!matcher.matches("bilibili.com", None, None, None), "exact only on www.bilibili.com");
}

#[test]
fn ipcidr_mrs_round_trip_matches_mihomo_semantics() {
    let body = load("sample_ipcidr.mrs");
    let compiled = parse_ruleset_compiled(RulesetFormat::Mrs, &body)
        .expect("parse sample_ipcidr.mrs");
    ...
    //
    // 命中：
    assert!(matcher.matches("10.1.2.3", Some(IpAddr::V4(Ipv4Addr::new(10, 1, 2, 3))), None, None));
    assert!(matcher.matches("172.20.30.40", Some(IpAddr::V4(Ipv4Addr::new(172, 20, 30, 40))), None, None));
    assert!(matcher.matches("192.168.0.1", Some(IpAddr::V4(Ipv4Addr::new(192, 168, 0, 1))), None, None));
    assert!(matcher.matches("1.1.1.1", Some(IpAddr::V4(Ipv4Addr::new(1, 1, 1, 1))), None, None));
    assert!(matcher.matches("8.8.8.8", Some(IpAddr::V4(Ipv4Addr::new(8, 8, 8, 8))), None, None));
    // IPv6
    ...
    // 不命中：
    assert!(!matcher.matches("8.8.4.4", Some(IpAddr::V4(Ipv4Addr::new(8, 8, 4, 4))), None, None));
    assert!(!matcher.matches("1.1.1.2", Some(IpAddr::V4(Ipv4Addr::new(1, 1, 1, 2))), None, None));
    assert!(!matcher.matches("11.0.0.1", Some(IpAddr::V4(Ipv4Addr::new(11, 0, 0, 1))), None, None));
