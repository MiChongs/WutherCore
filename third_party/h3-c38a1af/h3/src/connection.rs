        ...
        loop {
            if !self.stream.has_data() {
                match ready!(self.stream.poll_next(cx)) {
                    ...
                    Ok(Some(Frame::Data { .. })) if !self.stream.has_data() => {
                        // RFC 9114 permits a DATA frame with a zero-length
                        // payload. It carries no body bytes and is not the end
                        // of the HTTP message, so keep polling for the next
...
mod zero_length_data_tests {
    ...
    use crate::{
        frame::FrameStream,
        proto::frame::Frame,
        quic::{RecvStream, StreamErrorIncoming, StreamId},
        shared_state::SharedState,
        stream::BufRecvStream,
    };

    #[tokio::test]
    async fn zero_length_data_frame_is_not_message_eof() {
        let mut wire = BytesMut::new();
        Frame::Data(Bytes::new()).encode_with_payload(&mut wire);
        Frame::Data(Bytes::from_static(b"after-empty")).encode_with_payload(&mut wire);

        let recv = FakeRecv {
            chunks: VecDeque::from([wire.freeze()]),
        };
        let framed = FrameStream::<_, Bytes>::new(BufRecvStream::new(recv));
        let mut request =
            RequestStream::new(framed, u64::MAX, Arc::new(SharedState::default()), false);

        let mut body = future::poll_fn(|cx| request.poll_recv_data(cx))
            .await
            .expect("receive DATA")
            .expect("zero-length DATA must not be reported as EOF");
        assert_eq!(
            body.copy_to_bytes(body.remaining()),
            Bytes::from_static(b"after-empty")
        );
        assert!(future::poll_fn(|cx| request.poll_recv_data(cx))
            .await
            .expect("receive message EOF")
            .is_none());
    }
    ...
    impl RecvStream for FakeRecv {
        ...
        fn stop_sending(&mut self, _error_code: u64) {
            unreachable!("the receive-only regression test never stops the stream")
        }

        fn recv_id(&self) -> StreamId {
            unreachable!("the receive-only regression test never reads the stream id")
        }
    }
}
