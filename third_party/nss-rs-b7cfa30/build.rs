...
fn setup_standalone(nss_dir: String) -> Vec<String> {
    ...
    // WutherCore local patch. Upstream decides this by profile, so the only
    // way it ever links NSS statically outside Windows is a debug build --
    // which is why neqo's Android CI (`cargo ndk test`, a debug build) links
    // while a release build does not. Two of our release targets have no
    // shared NSS at all, so dynamic_link() would look for libnss3/libssl3/
    // libnssutil3 objects that their build never produces:
    //
    //   * Android: the one maintained cross build is
    //     mozilla/application-services' build-nss-android.sh, the same script
    //     Firefox for Android uses, and it emits static archives only.
    //   * musl: NSS is built from source against musl, and the archives are
    //     linked into a fully static binary, which rules out shared objects.
    //
    // Keying off the target rather than the profile makes release builds link
    // the same archives those toolchains actually produce.
    let target_os = env::var("CARGO_CFG_TARGET_OS").unwrap();
    let target_env = env::var("CARGO_CFG_TARGET_ENV").unwrap_or_default();
    let static_only_target = target_os == "android" || target_env == "musl";
